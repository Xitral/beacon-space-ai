(function patchResearchViewer() {
  ...
  function drawGroundTrack(snapshot) {
    ...
    ctx.fillStyle = "rgba(5,12,25,.92)";
    ctx.fillRect(0, 0, w, h);
    ctx.strokeStyle = "rgba(126,177,255,.18)";
    ctx.lineWidth = 1;
    ...
    function path(points, color) { ctx.strokeStyle = color; ctx.beginPath(); points.forEach((p, i) => { const q = project(p); if (i === 0) ctx.moveTo(q[0], q[1]); else ctx.lineTo(q[0], q[1]); }); ctx.stroke(); }
    path(snapshot.geometry.target_orbit_km || [], "rgba(87,165,255,.75)");
    path(snapshot.geometry.secondary_orbit_km || [], "rgba(255,184,77,.75)");
    for (const [p, color] of [[snapshot.geometry.target_position_km, "#57a5ff"], [snapshot.geometry.secondary_position_km, "#ffb84d"]]) { const q = project(p); ctx.fillStyle = color; ctx.beginPath(); ctx.arc(q[0], q[1], 4, 0, Math.PI * 2); ctx.fill(); }
  }
